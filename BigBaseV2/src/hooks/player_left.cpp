...
namespace big
{
	void hooks::player_leave(CNetworkObjectMgr* _this, CNetGamePlayer* net_player)
	{
		...
		{
			string msg = fmt::format(xorstr_("{} LEFT, freeing slot #{} with Rockstar ID: {}"), net_player_data->m_name, net_player->m_player_id, net_player_data->m_rockstar_id2);

			LOG(INFO) << msg;
			if (g_config.settings.notify_players)
				g_notification_service->push(xorstr_("Player Left"), msg);
		}
